...
///
/// On a fresh receipt the service, in ONE transaction: dedups on the idempotency key, mints the
/// gapless OR number (server-authoritative), issues + posts the balanced collection journal
/// (DR cash / CR collections-clearing), and stores the receipt with its key.
/// </summary>
    ...
    IUnitOfWork unitOfWork)
{
    public Task<ReceiptView> RecordReceiptAsync(
        ...
        unitOfWork.ExecuteInTransactionAsync(async token =>
        {
            ...
            var receipt = new OfficialReceipt(
                request.Payor, new Money(request.AmountPaid), mode, request.FundCluster,
                request.ReceivedAtUtc, request.PaidToAccount);
    ...
    private static ReceiptView ToView(OfficialReceipt r) => new(
        r.OrNumber!, r.Payor, r.AmountPaid.Amount, r.Mode.ToString(),
        r.FundCluster, r.PaidToAccount, r.ReceivedAt, r.IssuedAt, r.Status.ToString());
}
